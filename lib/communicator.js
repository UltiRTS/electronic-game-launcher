...
ipcserver.on('dMap', function(message, data){
	...
	try{md5=md5File.sync(window.wd+'/engine/maps/'+window.mapIndex[data][0])}
	catch{md5='NA'}
	...
	if (window.downloading.includes(data)){console.log('already downloading')}
		...
		downloadFile("http://ulti-repo.eterea.uk/dNTPDl/tmpMap/"+window.mapIndex[data][0],window.wd+"/engine/maps/"+window.mapIndex[data][0],function (){
		downloadFile("http://ulti-repo.eterea.uk/dNTPDl/tmpMap/"+data+'.png',window.wd+"/engine/maps/"+data+'.png',function (){message.reply('retrieved|'+data);removeFromDownloading(data)})
