...
function deletePathes(){
	...
	currentD=0
		for (zipName in archives_checksum){
			if (archives_checksum[zipName][0].endsWith(safedir)){continue}   //if a stress response if triggered by other archive mismatch, dont delete things in the safedir
		totalD+=1
...
function checkIndiMd5(){
	...
	var sourceMd5={}
	for (zipName in archives_checksum){
		if (archives_checksum[zipName][0].endsWith(safedir)){continue}   //do not check files in archives that's extracted straight on top of a safe dir
	...
	if (failed){eventEmitter.emit('downloadedDifferentArchives')}
